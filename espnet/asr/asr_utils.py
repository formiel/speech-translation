...
def parse_hypothesis(hyp, char_list, dkey="yseq"):
    """Parse hypothesis.
    Args:
    ...
    tokenid_as_list = list(map(int, hyp[dkey][1:]))
...
def add_results_to_json(js, nbest_hyps, char_list, output_idx=0):
    ...
    for n, hyp in enumerate(nbest_hyps, 1):
        ...
        # copy ground-truth
        if len(js["output"]) > 0:
            out_dic = dict(js["output"][output_idx].items())
        else:
...
def add_results_to_json_st_asr(js, nbest_hyps, char_list_tgt, char_list_src):
    ...
    new_js = dict()
    new_js['utt2spk'] = js['utt2spk']
    num_outputs = len(js['output'])
    new_js['output'] = [None] * num_outputs
    ...
    for n, hyp in enumerate(nbest_hyps, 1):
        ...
        # add recognition results
        out_dic_asr['rec_text'] = rec_text_asr
        out_dic_asr['rec_token'] = rec_token_asr
        out_dic_asr['rec_tokenid'] = rec_tokenid_asr
        out_dic_asr['score'] = score

        # add to list of N-best result dicts
        new_js['output'][1] = out_dic_asr

        # show 1-best result
        if n == 1:
            logging.info('groundtruth translation: %s' % out_dic['text'])
            logging.info('prediction translation: %s' % out_dic['rec_text'])
            logging.info('groundtruth asr: %s' % out_dic_asr['text'])
            logging.info('prediction asr: %s' % out_dic_asr['rec_text'])
